...
class DataFrameToTensor:
    ...
    def splittest(self, test_size,random_state,sample_frac):
        ...
        class_groups = slice_list['dataframe'].apply(self.isabnomal)

        # 根据 'class' 值为 0 和不为 0 将数据分成两个子集
        n_subset = slice_list[class_groups == 0]
        ab_subset = slice_list[class_groups != 0]
        if n_subset.shape[0] >= ab_subset.shape[0]*sample_frac:
            ...
            slice_list = pd.concat([n_subset,ab_subset])
